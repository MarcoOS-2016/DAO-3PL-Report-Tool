...
namespace DAO_3PL_Report_Tool
{
    public class ReportFileHandler
    {
        ...
        public ReportFileHandler(UserSelectedValue userSelectValue)
        {
        ...
        private void GetUnProcessedFileNameList()
        {
            ...
            try
            {                
                MiscUtility.LogHistory("Starting to search unprocessed 3PL files...");
                MainForm.backgroundworker.ReportProgress(0, string.Format("[{0}] - Starting to search unprocessed 3PL TRAN files...", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")));
                ...
                foreach (ThirdPartyFile thirdPartyFile in this.unprocessedfilenamelist)
                {
                    using (StreamReader reader = new FileInfo(thirdPartyFile.FullFillName).OpenText())
                    {
                        ...
                        if (thirdPartyFile.FileName.ToUpper().Contains("SNAP_3PL"))
                        {
                            ...
                            //Console.WriteLine(string.Format("[{0}] - Starting to read 3PL Tran text file - {1}...", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), thirdPartyFile.FullFillName));
                            MiscUtility.LogHistory(message);
                            MainForm.backgroundworker.ReportProgress(0, message);
